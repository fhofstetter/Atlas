...
export async function saveProducts(products) {
  ...
  await writeFile(PRODUCTS_FILE, JSON.stringify(data, null, 2), 'utf8')
}
...
export async function saveAlerts(alerts) {
  ...
  await writeFile(ALERTS_FILE, JSON.stringify(data, null, 2), 'utf8')
}
...
export async function appendHistory(productId, storeSnapshots) {
  ...
  await writeFile(file, JSON.stringify({ schema: '1.0', productId, observations: trimmed }, null, 2), 'utf8')
}
